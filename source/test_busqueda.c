...
void test_busqueda(int vector[], int *n_columnas)
{
    ...
    int posiciones[N_PRUEBAS_BUSQ][INDEX_CONOCIDOS]; // tabla donde se guarda las posiciones de los sistemas de ordenacion
    int i = 0, j, buscado;  // i para las filas, j para las columnas de intercambios[i][j]
    int num_tests, num_tests_ok;
    ...
    while(i < N_PRUEBAS_BUSQ){
        ...
        vector_ordenado(vector, n_columnas);
        ...
        while(j < INDEX_CONOCIDOS){
            ...
            posiciones[i][j] = busqueda_ite(vector, n_columnas, buscado); // se pasa el numero a buscar

            if (conocido[i][j] = posiciones[i][j]){ // Si la posicion buscada coincide con la conocida aumenta test_ok
                num_tests_ok++;
            }
